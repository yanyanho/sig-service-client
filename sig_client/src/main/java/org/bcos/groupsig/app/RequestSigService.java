...
public class RequestSigService {
	...
	public boolean linkableRingSig(SigStruct ringSigObj, String message, String ringName, String memberPos,
			int ringSize) {
		...
		subParamMap.put("message", message);
		if (ringSize > 0) {
			subParamMap.put("ring_size", ringSize);
		} else {
			System.out.println("ring_size must be larger than 0");
			return false;
		}
		paramMap.put("params", subParamMap);
